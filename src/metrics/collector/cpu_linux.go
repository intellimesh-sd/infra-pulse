...
var (
	/*enableCPUGuest       = flag.Bool("collector.cpu.guest", true, "Enables metric node_cpu_guest_seconds_total")
	enableCPUInfo        = flag.Bool("collector.cpu.info", false, "Enables metric cpu_info")
	...
	flagsInclude         = ""
	bugsInclude          = ""
	jumpBackDebugMessage = jumpBackSeconds
)
...
func NewCPUCollector() (Collector, error) {
	...
	if err != nil {
		...
		klog.Error("msg", "Could not open isolated file", "error", err)
	}
...
func (c *cpuCollector) compileIncludeFlags(flagsIncludeFlag, bugsIncludeFlag *string) error {
	if (*flagsIncludeFlag != "" || *bugsIncludeFlag != "") && !enableCPUInfo {
		enableCPUInfo = true
		klog.Info("msg", "--collector.cpu.info has been set to `true` because you set the following flags, like --collector.cpu.info.flags-include and --collector.cpu.info.bugs-include")
	}
...
func (c *cpuCollector) updateInfo(ch chan<- prometheus.Metric) error {
	...
	if !ok || cpuFreqEnabled == nil {
		klog.Info("msg", "cpufreq key missing or nil value in collectorState map")
...
func (c *cpuCollector) updateThermalThrottle(ch chan<- prometheus.Metric) error {
	...
	for _, cpu := range cpus {
		...
		if physicalPackageID, err = readUintFromFile(filepath.Join(cpu, "topology", "physical_package_id")); err != nil {
			klog.V(3).Info("msg", "CPU is missing physical_package_id", "cpu", cpu)
			continue
		}
		// topology/core_id
		if coreID, err = readUintFromFile(filepath.Join(cpu, "topology", "core_id")); err != nil {
			klog.V(3).Info("msg", "CPU is missing core_id", "cpu", cpu)
			continue
		}
		...
		if _, present := packageCoreThrottles[physicalPackageID][coreID]; !present {
			...
			if coreThrottleCount, err := readUintFromFile(filepath.Join(cpu, "thermal_throttle", "core_throttle_count")); err == nil {
				...
				klog.V(3).Info("msg", "CPU is missing core_throttle_count", "cpu", cpu)
			}
		}
		...
		if _, present := packageThrottles[physicalPackageID]; !present {
			...
			if packageThrottleCount, err := readUintFromFile(filepath.Join(cpu, "thermal_throttle", "package_throttle_count")); err == nil {
				...
				klog.V(3).Info("msg", "CPU is missing package_throttle_count", "cpu", cpu)
			}
		}
	}
...
func (c *cpuCollector) updateCPUStats(newStats map[int64]procfs.CPUStat) {
	...
	for i, n := range newStats {
		...
		if (cpuStats.Idle - n.Idle) >= jumpBackSeconds {
			klog.V(3).Info("msg", jumpBackDebugMessage, "cpu", i, "old_value", cpuStats.Idle, "new_value", n.Idle)
		...
		if n.Idle >= cpuStats.Idle {
			...
			klog.V(3).Info("msg", "CPU Idle counter jumped backwards", "cpu", i, "old_value", cpuStats.Idle, "new_value", n.Idle)
		}

		if n.User >= cpuStats.User {
			cpuStats.User = n.User
		} else {
			klog.V(3).Info("msg", "CPU User counter jumped backwards", "cpu", i, "old_value", cpuStats.User, "new_value", n.User)
		}

		if n.Nice >= cpuStats.Nice {
			cpuStats.Nice = n.Nice
		} else {
			klog.V(3).Info("msg", "CPU Nice counter jumped backwards", "cpu", i, "old_value", cpuStats.Nice, "new_value", n.Nice)
		}

		if n.System >= cpuStats.System {
			cpuStats.System = n.System
		} else {
			klog.V(3).Info("msg", "CPU System counter jumped backwards", "cpu", i, "old_value", cpuStats.System, "new_value", n.System)
		}

		if n.Iowait >= cpuStats.Iowait {
			cpuStats.Iowait = n.Iowait
		} else {
			klog.V(3).Info("msg", "CPU Iowait counter jumped backwards", "cpu", i, "old_value", cpuStats.Iowait, "new_value", n.Iowait)
		}

		if n.IRQ >= cpuStats.IRQ {
			cpuStats.IRQ = n.IRQ
		} else {
			klog.V(3).Info("msg", "CPU IRQ counter jumped backwards", "cpu", i, "old_value", cpuStats.IRQ, "new_value", n.IRQ)
		}

		if n.SoftIRQ >= cpuStats.SoftIRQ {
			cpuStats.SoftIRQ = n.SoftIRQ
		} else {
			klog.V(3).Info("msg", "CPU SoftIRQ counter jumped backwards", "cpu", i, "old_value", cpuStats.SoftIRQ, "new_value", n.SoftIRQ)
		}

		if n.Steal >= cpuStats.Steal {
			cpuStats.Steal = n.Steal
		} else {
			klog.V(3).Info("msg", "CPU Steal counter jumped backwards", "cpu", i, "old_value", cpuStats.Steal, "new_value", n.Steal)
		}

		if n.Guest >= cpuStats.Guest {
			cpuStats.Guest = n.Guest
		} else {
			klog.V(3).Info("msg", "CPU Guest counter jumped backwards", "cpu", i, "old_value", cpuStats.Guest, "new_value", n.Guest)
		}

		if n.GuestNice >= cpuStats.GuestNice {
			cpuStats.GuestNice = n.GuestNice
		} else {
			klog.V(3).Info("msg", "CPU GuestNice counter jumped backwards", "cpu", i, "old_value", cpuStats.GuestNice, "new_value", n.GuestNice)
		}
