...
func NewDiskstatsCollector() (Collector, error) {
	...
	if stat, err := os.Stat(udevDataPath); err != nil || !stat.IsDir() {
		klog.Error("msg", "Failed to open directory, disabling udev device properties", "path", udevDataPath)
	} else {
...
func (c *diskstatsCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, stats := range diskStats {
		...
		if err != nil {
			klog.V(3).Info("msg", "Failed to parse udev info", "err", err)
		}
		...
		if ata := info[udevIDATA]; ata != "" {
			for attr, desc := range c.ataDescs {
				str, ok := info[attr]
				if !ok {
					klog.V(3).Info("msg", "Udev attribute does not exist", "attribute", attr)
					continue
				}
				...
				if value, err := strconv.ParseFloat(str, 64); err == nil {
					...
					klog.Errorf("msg Failed to parse ATA value error:%s", err)
				}
			}
		}
	}
