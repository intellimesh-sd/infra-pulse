...
func (c *drbdCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			klog.V(3).Info("msg", "stats file does not exist, skipping", "file", statsFile, "err", err)
			return ErrNoData
		}
	...
	for scanner.Scan() {
		...
		if len(kv) != 2 {
			klog.V(3).Info("msg", "skipping invalid key:value pair", "field", field)
			continue
		}
		...
		klog.V(3).Info("msg", "unhandled key-value pair", "key", kv[0], "value", kv[1])
	}
