...
var (
	/*ethtoolDeviceInclude = flag.String("collector.ethtool.device-include", "",
		"Regexp of ethtool devices to include (mutually exclusive to device-exclude).")
	ethtoolDeviceExclude = flag.String("collector.ethtool.device-exclude", "",
		"Regexp of ethtool devices to exclude (mutually exclusive to device-include).")
	ethtoolIncludedMetrics = flag.String("collector.ethtool.metrics-include", ".*",
		"Regexp of ethtool stats to include.")*/
	ethtoolDeviceInclude   = ""
	ethtoolDeviceExclude   = ""
...
func (c *ethtoolCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) || errors.Is(err, os.ErrPermission) {
			klog.V(3).Info("msg", "Could not read netclass file", "err", err)
			return ErrNoData
		}
	...
	for device := range netClass {
		...
		if err == nil {
			...
			if errno, ok := err.(syscall.Errno); ok {
				if err == unix.EOPNOTSUPP {
					klog.V(3).Info("msg", "ethtool link info error", "err", err, "device", device, "errno", uint(errno))
				} else if errno != 0 {
					klog.V(3).Info("msg", "ethtool link info error", "err", err, "device", device, "errno", uint(errno))
				}
			} else {
				klog.V(3).Info("msg", "ethtool link info error", "err", err, "device", device)
			}
		}
		...
		if err == nil {
			...
			if errno, ok := err.(syscall.Errno); ok {
				if err == unix.EOPNOTSUPP {
					klog.V(3).Info("msg", "ethtool driver info error", "err", err, "device", device, "errno", uint(errno))
				} else if errno != 0 {
					klog.V(3).Info("msg", "ethtool driver info error", "err", err, "device", device, "errno", uint(errno))
				}
			} else {
				klog.V(3).Info("msg", "ethtool driver info error", "err", err, "device", device)
			}
		}
		...
		if err != nil {
			if errno, ok := err.(syscall.Errno); ok {
				if err == unix.EOPNOTSUPP {
					klog.V(3).Info("msg", "ethtool stats error", "err", err, "device", device, "errno", uint(errno))
				} else if errno != 0 {
					klog.V(3).Info("msg", "ethtool stats error", "err", err, "device", device, "errno", uint(errno))
				}
			} else {
				klog.V(3).Info("msg", "ethtool stats error", "err", err, "device", device)
			}
		}
		...
		for metric := range stats {
			...
			if exists {
				klog.V(3).Info("msg", "dropping duplicate metric name", "device", device,
