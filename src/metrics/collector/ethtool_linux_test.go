...
func NewTestEthtoolCollector() (prometheus.Collector, error) {
	dsc, err := NewEthtoolTestCollector()
	if err != nil {
...
func NewEthtoolTestCollector() (Collector, error) {
	collector, err := makeEthtoolCollector()
...
`
	*sysPath = "fixtures/sys"
	...
	collector, err := NewEthtoolTestCollector()
	if err != nil {
		panic(err)
	}
	c, err := NewTestEthtoolCollector()
	if err != nil {
		t.Fatal(err)
	}
