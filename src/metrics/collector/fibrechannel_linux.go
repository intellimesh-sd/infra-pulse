...
func (c *fibrechannelCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			klog.V(3).Info("msg", "fibrechannel statistics not found, skipping")
			return ErrNoData
		}
	...
	for _, host := range hosts {
		...
		ch <- prometheus.MustNewConstMetric(infoDesc, prometheus.GaugeValue, infoValue,
			*host.Name, *host.Speed, *host.PortState, *host.PortType, *host.PortID, *host.PortName,
			*host.FabricName, *host.SymbolicName, *host.SupportedClasses, *host.SupportedSpeeds, *host.DevLossTMO)
