...
/*
var mountTimeout = flag.Duration("collector.filesystem.mount-timeout", 5*time.Second,

	"how long to wait for a mount to respond before marking it as stale")

var statWorkerCount = flag.Int("collector.filesystem.stat-workers", 4,
	...
	go func() {
		for _, labels := range mps {
			if c.excludedMountPointsPattern.MatchString(labels.mountPoint) {
				klog.V(3).Info("msg", "Ignoring mount point", "mountpoint", labels.mountPoint)
				continue
			}
			if c.excludedFSTypesPattern.MatchString(labels.fsType) {
				klog.V(3).Info("msg", "Ignoring fs", "type", labels.fsType)
				continue
			}
			...
			if _, ok := stuckMounts[labels.mountPoint]; ok {
				...
				klog.V(3).Info("msg", "Mount point is in an unresponsive state", "mountpoint", labels.mountPoint)
...
func (c *filesystemCollector) processStat(labels filesystemLabels) filesystemStats {
	...
	if _, ok := stuckMounts[labels.mountPoint]; ok {
		klog.V(3).Info("msg", "Mount point has recovered, monitoring will resume", "mountpoint", labels.mountPoint)
	...
	if err != nil {
		...
		klog.V(3).Info("msg", "Error on statfs() system call", "rootfs", rootfsFilePath(labels.mountPoint), "err", err)
...
func stuckMountWatcher(mountPoint string, success chan struct{}) {
	...
	case <-mountCheckTimer.C:
		...
		default:
			klog.V(3).Info("msg", "Mount point timed out, it is being labeled as stuck and will not be monitored", "mountpoint", mountPoint)
...
func mountPointDetails() ([]filesystemLabels, error) {
	...
	if errors.Is(err, os.ErrNotExist) {
		...
		klog.V(3).Info("msg", "Reading root mounts failed, falling back to system mounts", "err", err)
