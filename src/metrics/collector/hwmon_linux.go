...
var (
	/*collectorHWmonChipInclude = flag.String("collector.hwmon.chip-include", "",
		"Regexp of hwmon chip to include (mutually exclusive to device-exclude).")
	collectorHWmonChipExclude = flag.String("collector.hwmon.chip-exclude", "",
		"Regexp of hwmon chip to exclude (mutually exclusive to device-include).")*/
	collectorHWmonChipInclude = ""
	collectorHWmonChipExclude = ""
...
func (c *hwMonCollector) updateHwmon(ch chan<- prometheus.Metric, dir string) error {
	...
	if c.deviceFilter.ignored(hwmonName) {
		klog.V(3).Info("msg", "ignoring hwmon chip", "chip", hwmonName)
		return nil
	}
...
func (c *hwMonCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			klog.V(3).Info("msg", "hwmon collector metrics are not available for this system")
			return ErrNoData
		}
