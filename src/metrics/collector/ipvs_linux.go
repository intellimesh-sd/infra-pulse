...
var (
	...
	/*ipvsLabels = flag.String("collector.ipvs.backend-labels", strings.Join(fullIpvsBackendLabels, ","),
...
func (c *ipvsCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		...
		if errors.Is(err, os.ErrNotExist) {
			klog.V(3).Info("msg", "ipvs collector metrics are not available for this system")
			return ErrNoData
		}
