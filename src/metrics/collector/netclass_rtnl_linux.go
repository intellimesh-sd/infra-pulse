...
var (
	/*netclassRTNLWithStats = flag.Bool("collector.netclass_rtnl.with-stats", false,
	"Expose the statistics for each network device, replacing netdev collector.")*/
	netclassRTNLWithStats = false
	operstateStr          = []string{
...
func (c *netClassCollector) netClassRTNLUpdate(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		...
		klog.Info("msg ETHTOOL netlink interface unavailable, duplex and linkspeed are not scraped.")
	} else {
		for _, lm := range lms {
	...
	for _, msg := range relevantLinks {
		...
		if !netclassRTNLWithStats || msg.Attributes.Stats64 == nil {
			continue
		}
