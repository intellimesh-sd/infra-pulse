...
var (
	//netDevNetlink = flag.Bool("collector.netdev.netlink", true, "Use netlink to gather stats instead of /proc/net/dev.")
	netDevNetlink = true
)
...
func parseNetlinkStats(links []rtnetlink.LinkMessage, filter *deviceFilter) netDevStats {
	...
	for _, msg := range links {
		if msg.Attributes == nil {
			klog.V(3).Info("msg", "No netlink attributes, skipping")
			continue
		}
		...
		if filter.ignored(name) {
			klog.V(3).Info("msg", "Ignoring device", "device", name)
			continue
		}

		// Make sure we don't panic when accessing `stats` attributes below.
		if stats == nil {
			klog.V(3).Info("msg", "No netlink stats, skipping")
			continue
		}
