...
var (
	perfSwProfilerFlag = make([]string, 0)
	perfHwProfilerFlag = make([]string, 0)
	perfCaProfilerFlag = make([]string, 0)
	perfTracepointFlag = make([]string, 0)
	/*perfCPUsFlag       = flag.String("collector.perf.cpus", "", "Statuses of CPUs from which perf metrics should be collected")

	perfNoHwProfiler = flag.Bool("collector.perf.disable-hardware-profilers", false, "disable perf hardware profilers")
	perfNoSwProfiler = flag.Bool("collector.perf.disable-software-profilers", false, "disable perf software profilers")
	perfNoCaProfiler = flag.Bool("collector.perf.disable-cache-profilers", false, "disable perf cache profilers")
	*/
	perfCPUsFlag     = ""
	perfNoHwProfiler = false
	perfNoSwProfiler = false
	perfNoCaProfiler = false
	/*perfSwProfilerFlag = flag.Var("collector.perf.software-profilers", "perf software profilers that should be collected").Strings()
	perfHwProfilerFlag = flag.Bool("collector.perf.hardware-profilers", "perf hardware profilers that should be collected").Strings()
	perfTracepointFlag = flag.String("collector.perf.tracepoint", "", "perf tracepoint that should be collected")
	perfCaProfilerFlag = flag.Bool("collector.perf.cache-profilers", "perf cache profilers that should be collected").Strings()*/
)
...
func (c *perfTracepointCollector) updateCPU(cpu int, ch chan<- prometheus.Metric) error {
	...
	if err := profiler.Profile(p); err != nil {
		klog.Errorf("msg Failed to collect tracepoint profile error:%s", err)
		return err
	}
