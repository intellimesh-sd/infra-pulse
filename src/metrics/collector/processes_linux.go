	...
	for _, pid := range p {
		...
		if err != nil {
			...
			if c.isIgnoredError(err) {
				klog.V(3).Info("msg", "file not found when retrieving stats for pid", "pid", pid.PID, "err", err)
				continue
			}
			klog.V(3).Info("msg", "error reading stat for pid", "pid", pid.PID, "err", err)
...
func (c *processCollector) getThreadStates(pid int, pidStat procfs.ProcStat, threadStates map[string]int32) error {
	...
	if err != nil {
		if c.isIgnoredError(err) {
			klog.V(3).Info("msg", "file not found when retrieving tasks for pid", "pid", pid, "err", err)
			return nil
		}
		klog.V(3).Info("msg", "error reading tasks for pid", "pid", pid, "err", err)
	...
	for _, thread := range t {
		...
		if err != nil {
			if c.isIgnoredError(err) {
				klog.V(3).Info("msg", "file not found when retrieving stats for thread", "pid", pid, "threadId", thread.PID, "err", err)
				continue
			}
			klog.V(3).Info("msg", "error reading stat for thread", "pid", pid, "threadId", thread.PID, "err", err)
