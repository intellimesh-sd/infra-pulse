...
var (
	//raplZoneLabel = flag.Bool("collector.rapl.enable-zone-label", false, "Enables service unit metric unit_start_time_seconds")
	raplZoneLabel = false
)
...
func (c *raplCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			klog.V(3).Info("msg", "Platform doesn't have powercap files present", "err", err)
			return ErrNoData
		}
		if errors.Is(err, os.ErrPermission) {
			klog.V(3).Info("msg", "Can't access powercap files", "err", err)
			return ErrNoData
		}
	...
	for _, rz := range zones {
		...
		if err != nil {
			if errors.Is(err, os.ErrPermission) {
				klog.V(3).Info("msg", "Can't access energy_uj file", "zone", rz, "err", err)
				return ErrNoData
			}
			return err
		}
