...
var (
	// ignoredTapeDevices = flag.String("collector.tapestats.ignored-devices", "^$", "Regexp of devices to ignore for tapestats.")
	ignoredTapeDevices = "^$"
)
...
func (c *tapestatsCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		if os.IsNotExist(err) {
			klog.V(3).Info("msg", "scsi_tape stats not found, skipping")
			return ErrNoData
		}
	...
	for _, tape := range tapes {
		if c.ignoredDevicesPattern.MatchString(tape.Name) {
			klog.V(3).Info("msg", "Ignoring device", "device", tape.Name)
			continue
		}
