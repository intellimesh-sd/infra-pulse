...
func convertMetricFamily(metricFamily *dto.MetricFamily, ch chan<- prometheus.Metric) {
	...
	for _, metric := range metricFamily.Metric {
		if metric.TimestampMs != nil {
			klog.V(2).Info("msg", "Ignoring unsupported custom timestamp on textfile collector metric", "metric", metric)
		}
...
func (c *textFileCollector) Update(ch chan<- prometheus.Metric) error {
	...
	for _, path := range paths {
		...
		if err != nil && path != "" {
			errored = true
			klog.Error("msg", "failed to read textfile collector directory", "path", path, "err", err)
		}
		...
		for _, f := range files {
			...
			for _, mf := range families {
				...
				if helpTexts, seen := metricsNamesToHelpTexts[*mf.Name]; seen {
					if mf.Help != nil && helpTexts[0] != *mf.Help || helpTexts[1] != "" {
						...
						errored = true
						klog.Error("msg", "inconsistent metric help text",
							"metric", *mf.Name,
			...
			if err != nil {
				errored = true
				klog.Error("msg", "failed to collect textfile data", "file", f.Name(), "err", err)
				continue
			}
