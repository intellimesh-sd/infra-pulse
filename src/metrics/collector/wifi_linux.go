...
func (c *wifiCollector) Update(ch chan<- prometheus.Metric) error {
	...
	if err != nil {
		...
		if errors.Is(err, os.ErrNotExist) {
			klog.V(3).Info("msg", "wifi collector metrics are not available for this system")
			return ErrNoData
		}
		if errors.Is(err, os.ErrPermission) {
			klog.V(3).Info("msg", "wifi collector got permission denied when accessing metrics")
			return ErrNoData
		}
	...
	for _, ifi := range ifis {
		...
		klog.V(3).Info("msg", "probing wifi device with type", "wifi", ifi.Name, "type", ifi.Type)
		...
		case errors.Is(err, os.ErrNotExist):
			klog.V(3).Info("msg", "BSS information not found for wifi device", "name", ifi.Name)
		default:
		...
		case errors.Is(err, os.ErrNotExist):
			klog.V(3).Info("msg", "station information not found for wifi device", "name", ifi.Name)
		default:
