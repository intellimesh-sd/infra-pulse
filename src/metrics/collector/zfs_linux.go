...
func (c *zfsCollector) openProcFile(path string) (*os.File, error) {
	...
	if err != nil {
		...
		klog.V(3).Info("msg", "Cannot open file for reading", "path", procFilePath(path))
...
func (c *zfsCollector) updatePoolStats(ch chan<- prometheus.Metric) error {
	...
	for _, zpoolPath := range zpoolPaths {
		...
		if err != nil {
			...
			klog.V(3).Info("msg", "Cannot open file for reading", "path", zpoolPath)
	...
	if zpoolStatePaths == nil {
		klog.V(3).Info("msg", "No pool state files found")
		return nil
	}
